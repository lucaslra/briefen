...
/**
 * Handles all Readeck proxy logic — settings loading, HTTP calls, content extraction.
 * The API key never leaves the server; the frontend only sees this service's responses.
 */
@Service
public class ReadeckService {

    private static final Logger log = LoggerFactory.getLogger(ReadeckService.class);

    private final SettingsPersistence settingsPersistence;
    private final JsonMapper objectMapper;
    private final HttpClient httpClient;

    public ReadeckService(SettingsPersistence settingsPersistence, JsonMapper objectMapper) {
    ...
    public String listBookmarks(String userId, int page, int limit, String search, String status) {
        var settings = requireReadeck(userId);
    ...
    public String getBookmark(String userId, String id) {
        sanitizeId(id);
        var settings = requireReadeck(userId);
        return proxyGetWithRetry(settings.getReadeckUrl() + "/api/bookmarks/" + id, settings.getReadeckApiKey());
    }

    public Map<String, String> getArticleContent(String userId, String id) {
        sanitizeId(id);
        var settings = requireReadeck(userId);
    ...
    private UserSettings loadSettings(String userId) {
        return settingsPersistence.findByUserId(userId).orElseGet(UserSettings::new);
    }
     ...
     */
    public static void validateReadeckUrl(String url) {
        try {
